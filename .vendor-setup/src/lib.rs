...
pub fn as_self_contained() {
    ...
    let runtime_dll = dest.join("Microsoft.WindowsAppRuntime.dll");
    if !runtime_dll.is_file() {
        panic!(
            "self-contained WASDK {RUNTIME_VER} staging failed: missing {}",
            runtime_dll.display()
        );
    }
...
fn dl_nupkg(name: &str, ver: &str, dest: &Path) {
    ...
    let curl = env::var_os("SystemRoot")
        ...
        .filter(|p| p.is_file());
    let output = curl.and_then(|c| {
        Command::new(&c)
            .args(["-f", "-s", "-L", "-o", dest.to_str().unwrap(), &url])
            .output()
            .ok()
    });
    let ok = output
        ...
        && dest.is_file()
        && dest.metadata().map(|m| m.len() > 1_000_000).unwrap_or(false);
    if !ok {
        let _ = fs::remove_file(dest);
        panic!("failed to download {name} {ver} from {url}");
    }
