...
use super::convert::{string_as_checkbox_content, string_as_textblock};
use super::*;
...
pub fn dispatch(handle: &Handle, prop: Prop, value: &PropValue) -> Result<bool> {
    match (prop, value, handle) {
        ...
        (Prop::Content, PropValue::Str(v), Handle::CheckBox(_)) => {
            let tb = string_as_checkbox_content(v.as_str())?;
            handle
                .cast_inner::<bindings::IContentControl>()?
                .SetContent(&tb)?;
        }
        ...
        (Prop::Time, PropValue::I32(v), Handle::TimePicker(h)) => {
            h.SetTime(windows_time::TimeSpan::from_ticks(
                i64::from(*v) * 60 * 10_000_000,
            ))?;
        }
